...
bool BootInfo::eraseAppArea()
{
	FLASH_EraseInitTypeDef eraseInits;
	uint32_t errors;

	uint32_t startPage = getPage(appStartAddress);
	uint32_t numberPages = (getPage(getEndAddress()) - startPage) / pageSize;

	eraseInits.TypeErase = FLASH_TYPEERASE_PAGES;
	eraseInits.PageAddress = startPage;
	eraseInits.NbPages = numberPages;

	return (HAL_FLASHEx_Erase(&eraseInits, &errors) == HAL_OK);
}
...
void BootInfo::unlock()
{
	HAL_FLASH_Unlock();
}

bool BootInfo::writeBlock(uint32_t addr, const uint8_t* data, uint32_t size)
{
	for (uint32_t i = 0; i < size; i += sizeof(uint32_t)) {
		uint32_t address = addr + i;
		uint32_t wordToWrite = *(uint32_t*)(data + i);

		auto status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, address, wordToWrite);
...
uint32_t BootInfo::getEndAddress()
{
	return ADDR_FLASH_PAGE_127 + pageSize - 1;
}

uint32_t BootInfo::getPage(uint32_t address)
{
	uint32_t sector = 0;
...
uint16_t BootInfo::crc16(const uint8_t* data, uint32_t size)
{
	uint16_t crc = 0;
