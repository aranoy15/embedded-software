      ...
      mPacketManager(new PacketManager(1024))
{
    ...
    connect(this, &UploadExecution::testFinished, this, &UploadExecution::exitFromApplication);
    connect(this, &UploadExecution::updateStageSignal, this, &UploadExecution::updateStage);
...
void UploadExecution::startExec(const QString& port, const QString& filePath, int baud)
{
...
void UploadExecution::updateStage(int stage)
{
    ...
    float percent = (100.0f * stage) / static_cast<double>(mPacketManager->packetCount());
	cout << "\r" << fixed << setprecision(1) << setw(8) << percent << "%" << flush;
    ...
    float totalSize = ((mPacketManager->packetSize() * mPacketManager->packetCount()) / 1024.0f) - countKbSended;
